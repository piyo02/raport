...
class Students_model extends MY_Model
{
  ...
  public function students_by_school_id( $start = 0 , $limit = NULL, $school_id = NULL )
  {
      if (isset( $limit ))
      {
        $this->limit( $limit );
  ...
  public function students_by_classroom_id( $start = 0 , $limit = NULL, $classroom_id = NULL )
  {
      if (isset( $limit ))
      {
        $this->limit( $limit );
      ...
      if($classroom_id)
        $this->where('classroom_id', $classroom_id);
      $this->offset( $start );
      $this->order_by($this->table.'.id', 'asc');
      return $this->fetch_data();
  }
